...
const Wheel: React.FC<WheelProps> = ({
  ...
  const segmentAngle = 360 / candyOptions.length;
  ...
  return (
    <div className="relative flex flex-col items-center w-full max-w-[95vw] sm:max-w-none">
      ...
      <div className="wheel-container relative shadow-[0_0_50px_rgba(0,0,0,0.8)] rounded-full border-[8px] sm:border-[12px] border-[#2D2418] bg-[#1a130c] overflow-hidden aspect-square w-full sm:w-[500px] h-auto sm:h-[500px]">
        <svg
        ...
        <button
          onClick={onSpin}
          disabled={isSpinning}
          className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-20 h-20 sm:w-28 sm:h-28 rounded-full border-4 border-[#433422] bg-[#f2e8c9] shadow-[0_0_30px_rgba(242,232,201,0.5)] flex items-center justify-center group transition-all duration-300 z-30 ${
            isSpinning
        ...
        >
          <div className="text-[#433422] font-black text-center leading-none">
            <span className="text-[8px] sm:text-[10px] block mb-1 opacity-70 uppercase tracking-tighter">
              Invocação
            </span>
            <span className="text-sm sm:text-xl tracking-widest">
              {isSpinning ? "..." : "GIRAR"}
            </span>
          </div>
        </button>
